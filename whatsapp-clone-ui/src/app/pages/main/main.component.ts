...
export class MainComponent implements OnInit, OnDestroy, AfterViewChecked {
  ...
  private setMessagesToSeen() {
    this.messageService
      .setMessagesToSeen({'chat-id': this.selectedChat.id as string})
      .subscribe({
        next: () => {
        }
      })
  ...
  private getReceiverId(): string {
    if (this.selectedChat.receiverId === this.keycloakService.userId) {
      return this.selectedChat.receiverId as string;
    }
    return this.selectedChat.senderId as string;
  }
  ...
  private initWebSocket() {
    if (this.keycloakService.keycloak.tokenParsed?.sub) {
      ...
      this.socketClient = Stopm.over(ws);
      const subUrl = `/user/${this.keycloakService.keycloak.tokenParsed?.sub}/chat`;
      this.socketClient.connect({'Authorization': 'Bearer ' + this.keycloakService.keycloak.token},
        () => {
          this.notificationSubscription = this.socketClient(subUrl,
            (message: any) => {
